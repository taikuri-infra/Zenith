...
const engineBadge: Record<string, { label: string; className: string }> = {
  ...
  rabbitmq: { label: "Q", className: "bg-purple-500/20 text-purple-400" },
  kafka: { label: "K", className: "bg-orange-500/20 text-orange-400" },
};
    ...
    <Shell>
      ...
      {createdDb && (() => {
        ...
        const handleDownloadEnv = () => {
          const urlKey = createdDb.engine === "redis" ? "REDIS_URL" : createdDb.engine === "mysql" ? "MYSQL_URL" : createdDb.engine === "mongodb" ? "MONGODB_URL" : createdDb.engine === "rabbitmq" ? "RABBITMQ_URL" : createdDb.engine === "kafka" ? "KAFKA_URL" : "DATABASE_URL";
        const envContent = [
