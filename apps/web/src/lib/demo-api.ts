...
const mockDeployApps: DeployApp[] = [
  ...
  },
  {
    ...
    framework: "go",
    status: "running",
    subdomain: "go-api-b2e1",
    port: 8080,
    url: "https://go-api-b2e1.freezenith.com",
    app_type: "web",
    exposure: "protected",
    auto_gateway_id: "gw-demo-1",
...
const demoAuthPools = {
  ...
  signup: async () => { await delay(300); return { user: { id: "u-1", email: "demo@test.com", firstName: "", lastName: "", enabled: true, createdTimestamp: Date.now() }, access_token: "demo-token", refresh_token: "demo-refresh", expires_in: 300 }; },
  login: async () => { await delay(300); return { access_token: "demo-token", refresh_token: "demo-refresh", expires_in: 300, token_type: "Bearer" }; },
  logout: async () => { await delay(300); return { message: "logged out" }; },
  refresh: async () => { await delay(300); return { access_token: "demo-token", refresh_token: "demo-refresh", expires_in: 300, token_type: "Bearer" }; },
  forgotPassword: async () => { await delay(300); return { message: "sent" }; },
  resetPassword: async () => { await delay(300); return { message: "reset" }; },
  revealSecret: async () => { await delay(300); return { client_secret: "demo-secret" }; },
  ...
  sendMagicLink: async () => { await delay(300); return { message: "sent" }; },
  ...
  sendOTP: async () => { await delay(300); return { message: "OTP sent", expires_in: 300 }; },
