...
function CallbackInner() {
  ...
  useEffect(() => {
    ...
    api.auth
      ...
      .then(() => {
        // Check that the user has admin/owner role
        ...
        if (token) {
          try {
            ...
            if (role !== "owner" && role !== "admin") {
              // Not an admin — clear tokens and redirect with error
              clearTokens();
              router.replace("/login?error=not_admin");
              return;
            }
          } catch {
            // If we can't parse, let them through — server will reject on API calls
          }
        }
        router.replace("/");
      })
