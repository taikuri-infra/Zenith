...
export default function NetworkingPage() {
  ...
  const routes = useApi<Route[]>(() => apiClient.networking.routes());
  const certs = useApi<Certificate[]>(() => apiClient.networking.certificates());
  const dns = useApi<DnsRecord[]>(() => apiClient.networking.dnsRecords());
  ...
  return (
    <Shell>
      <div className="space-y-6">
        ...
        {/* Routes */}
        <section>
          <h2 className="mb-3 text-sm font-medium text-white">Routes</h2>
          {routes.loading ? (
            ...
            <EmptyState
              title="No routes"
              description="No ingress routes configured."
              icon={Network}
            />
          ) : (
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {routes.data.map((route) => (
                    <tr key={route.name} className="border-b border-border last:border-0 transition-colors hover:bg-surface-200">
        ...
        {/* Certificates */}
        <section>
          <h2 className="mb-3 text-sm font-medium text-white">Certificates</h2>
          {certs.loading ? (
            ...
            <EmptyState
              ...
              description="No TLS certificates found."
              icon={Shield}
            />
          ) : (
            <div className="overflow-hidden rounded-lg border border-border">
                    ...
                    <tr key={cert.name} className="border-b border-border last:border-0 transition-colors hover:bg-surface-200">
                      ...
                      <td className="px-4 py-3">
                        <div className="flex flex-wrap gap-1">
                          {cert.domains.map((d) => (
                            <span key={d} className="rounded bg-surface-300 px-1.5 py-0.5 font-mono text-[10px] text-neutral-300">
                              {d}
                            </span>
                          ))}
                        </div>
        ...
        <section>
          ...
          {dns.loading ? (
            ...
            <EmptyState
              ...
              description="No external DNS records configured."
              icon={Globe}
            />
          ) : (
            <div className="overflow-hidden rounded-lg border border-border">
