...
- Keep `docker-compose.yml` and `infra/scripts/install.sh` for local standalone mode (unaffected)
...
- Affected code: `infra/scripts/deploy.sh` (replaced), `infra/scripts/cloudflare-dns.sh` (replaced by Terraform orchestration), `infra/k8s/keda/install.sh` (replaced)
- New directory: `infra/ansible/`
...
- Existing `infra/k8s/` manifests reused as templates within Ansible roles
- Existing `infra/helm/` charts can be adopted by Ansible roles via `kubernetes.core.helm`
