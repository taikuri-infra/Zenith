...
func TestGatewayRouteDeepCopyWithAllFields(t *testing.T) {
	route := &GatewayRoute{
		ObjectMeta: metav1.ObjectMeta{Name: "full-route", Namespace: "ns"},
		Spec: GatewayRouteSpec{
			Path:    "/api/v1/users",
			Methods: []string{"GET", "POST", "PUT"},
			Service: ServiceRef{Name: "user-svc", Port: 8080},
	...
	// Verify Plugins independence
	if len(copied.Spec.Plugins) != 1 {
		t.Fatalf("DeepCopy failed: Plugins length = %d", len(copied.Spec.Plugins))
	}
	copied.Spec.Plugins[0].Config["key1"] = "changed"
	if route.Spec.Plugins[0].Config["key1"] == "changed" {
		t.Error("DeepCopy is not independent for Plugin Config map")
	}

	// Verify Auth independence
	if copied.Spec.Auth == nil {
		t.Fatal("DeepCopy failed: Auth is nil")
	}
	copied.Spec.Auth.Scopes[0] = "changed"
	if route.Spec.Auth.Scopes[0] == "changed" {
		t.Error("DeepCopy is not independent for Auth.Scopes")
	}

	// Verify RateLimit independence
	if copied.Spec.RateLimit == nil {
		t.Fatal("DeepCopy failed: RateLimit is nil")
	}
	copied.Spec.RateLimit.RequestsPerSecond = 999
	if route.Spec.RateLimit.RequestsPerSecond == 999 {
		t.Error("DeepCopy is not independent for RateLimit pointer")
	}

	// Verify CORS independence
	if copied.Spec.CORS == nil {
		t.Fatal("DeepCopy failed: CORS is nil")
	}
	copied.Spec.CORS.AllowedOrigins[0] = "https://changed.com"
	if route.Spec.CORS.AllowedOrigins[0] == "https://changed.com" {
		t.Error("DeepCopy is not independent for CORS.AllowedOrigins")
	}
}

func TestGatewayRouteDeepCopyWithNilOptionalFields(t *testing.T) {
	route := &GatewayRoute{
		ObjectMeta: metav1.ObjectMeta{Name: "minimal-route"},
		Spec: GatewayRouteSpec{
			Path:      "/api",
			Service:   ServiceRef{Name: "svc", Port: 80},
			Methods:   nil,
			Plugins:   nil,
			Auth:      nil,
			RateLimit: nil,
	...
	copied := route.DeepCopy()
	if copied.Spec.Methods != nil {
		t.Error("DeepCopy should preserve nil Methods")
	}
	if copied.Spec.Plugins != nil {
		t.Error("DeepCopy should preserve nil Plugins")
	}
	if copied.Spec.Auth != nil {
		t.Error("DeepCopy should preserve nil Auth")
	}
	if copied.Spec.RateLimit != nil {
		t.Error("DeepCopy should preserve nil RateLimit")
	}
	if copied.Spec.CORS != nil {
		t.Error("DeepCopy should preserve nil CORS")
	}
}

func TestGatewayRouteDeepCopyInto(t *testing.T) {
	route := &GatewayRoute{
	...
	out := &GatewayRoute{}
	route.DeepCopyInto(out)
	if out.Spec.Path != "/api" {
		t.Errorf("DeepCopyInto failed: Path = %q", out.Spec.Path)
	}
	if out.Spec.Service.Name != "svc" {
		t.Errorf("DeepCopyInto failed: Service.Name = %q", out.Spec.Service.Name)
	}
}

func TestGatewayRouteDeepCopyObject(t *testing.T) {
	route := &GatewayRoute{ObjectMeta: metav1.ObjectMeta{Name: "route"}}
	obj := route.DeepCopyObject()
	_, ok := obj.(*GatewayRoute)
	if !ok {
		t.Fatal("DeepCopyObject did not return *GatewayRoute")
	}
}

func TestGatewayRouteListDeepCopy(t *testing.T) {
	list := &GatewayRouteList{
		Items: []GatewayRoute{{ObjectMeta: metav1.ObjectMeta{Name: "r1"}}},
	}
	copied := list.DeepCopy()
	if len(copied.Items) != 1 {
		t.Fatalf("DeepCopy failed: Items length = %d", len(copied.Items))
...
func TestGatewayRouteListDeepCopyObject(t *testing.T) {
	list := &GatewayRouteList{Items: []GatewayRoute{{ObjectMeta: metav1.ObjectMeta{Name: "r1"}}}}
	obj := list.DeepCopyObject()
	_, ok := obj.(*GatewayRouteList)
	if !ok {
		t.Fatal("DeepCopyObject did not return *GatewayRouteList")
	}
}

func TestGatewayRouteSpecDeepCopy(t *testing.T) {
	spec := &GatewayRouteSpec{
		Path:    "/api",
		Methods: []string{"GET"},
		Service: ServiceRef{Name: "svc", Port: 80},
		Auth:    &RouteAuth{Enabled: true, Scopes: []string{"read"}},
	}
	copied := spec.DeepCopy()
	copied.Auth.Enabled = false
	if !spec.Auth.Enabled {
		t.Error("DeepCopy is not independent for Auth")
	}
}

func TestGatewayRouteSpecDeepCopyNil(t *testing.T) {
	var spec *GatewayRouteSpec
	if spec.DeepCopy() != nil {
		t.Error("DeepCopy of nil GatewayRouteSpec should return nil")
	}
}

func TestGatewayRouteStatusDeepCopy(t *testing.T) {
	status := &GatewayRouteStatus{
		Phase:       "Active",
		KongRouteID: "id",
		Conditions:  []metav1.Condition{{Type: "Ready"}},
...
func TestGatewayRouteStatusDeepCopyNil(t *testing.T) {
	var status *GatewayRouteStatus
	if status.DeepCopy() != nil {
		t.Error("DeepCopy of nil GatewayRouteStatus should return nil")
	}
}

func TestGatewayPluginDeepCopyInto(t *testing.T) {
	...
	out := &GatewayPlugin{}
	plugin.DeepCopyInto(out)

	out.Config["second"] = "200"
	if plugin.Config["second"] == "200" {
		t.Error("DeepCopyInto is not independent for Config")
	}
}

func TestGatewayPluginDeepCopyIntoNilConfig(t *testing.T) {
	plugin := &GatewayPlugin{
		Name:   "simple",
		Config: nil,
	}
...
func TestRouteAuthDeepCopyInto(t *testing.T) {
	auth := &RouteAuth{
		Enabled: true,
		Scopes:  []string{"read", "write"},
	}
	out := &RouteAuth{}
	auth.DeepCopyInto(out)

	out.Scopes[0] = "changed"
	if auth.Scopes[0] == "changed" {
		t.Error("DeepCopyInto is not independent for Scopes")
	}
}

func TestRouteAuthDeepCopyIntoNilScopes(t *testing.T) {
	auth := &RouteAuth{Enabled: true, Scopes: nil}
	out := &RouteAuth{}
	auth.DeepCopyInto(out)
	if out.Scopes != nil {
		t.Error("DeepCopyInto should preserve nil Scopes")
...
func TestRouteCORSDeepCopyInto(t *testing.T) {
	cors := &RouteCORS{
		AllowedOrigins: []string{"https://example.com"},
		AllowedMethods: []string{"GET", "POST"},
	...
	out := &RouteCORS{}
	cors.DeepCopyInto(out)

	out.AllowedOrigins[0] = "changed"
	if cors.AllowedOrigins[0] == "changed" {
		t.Error("DeepCopyInto is not independent for AllowedOrigins")
	}
	out.AllowedMethods[0] = "DELETE"
	if cors.AllowedMethods[0] == "DELETE" {
		t.Error("DeepCopyInto is not independent for AllowedMethods")
	}
	out.AllowedHeaders[0] = "X-Custom"
	if cors.AllowedHeaders[0] == "X-Custom" {
		t.Error("DeepCopyInto is not independent for AllowedHeaders")
	}
}

func TestRouteCORSDeepCopyIntoNilSlices(t *testing.T) {
	cors := &RouteCORS{
		AllowedOrigins: nil,
		AllowedMethods: nil,
		AllowedHeaders: nil,
...
func TestNilDeepCopy(t *testing.T) {
	...
	if route.DeepCopy() != nil {
		t.Error("DeepCopy of nil GatewayRoute should return nil")
	}
...
func TestNilListDeepCopy(t *testing.T) {
	...
	var routeList *GatewayRouteList
	if routeList.DeepCopy() != nil {
		t.Error("DeepCopy of nil GatewayRouteList should return nil")
	}
