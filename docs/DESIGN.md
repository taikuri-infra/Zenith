# Zenith - Product Architecture & Strategy
...
- [ ] Zenith API skeleton (FastAPI, same Lich arch as FairBroker)
...
|-----------|------|---------|
| Zenith API | FastAPI (Python) | All platform operations |
| Zenith Web | Next.js 15 | Dashboard, deploy UI, billing |
...
| DNS | Cloudflare API (or Hetzner DNS) | Auto DNS for *.zenith.dev |
...
| Resource | Hetzner Price | Our Markup | Zenith Price |
|----------|--------------|------------|--------------|
...
Customer clicks "Add a Planet" (add node)
  -> Zenith API -> Hetzner API: create VM
...
### Zenith vs Competition

| Feature | Fly.io | Railway | Render | Zenith |
|---------|--------|---------|--------|--------|
...
> **Zenith**: The developer cloud that doesn't charge cloud prices.
>
...
| CLI tool (`zenith deploy`) | Web UI + API is enough for launch. | Phase 2 |
