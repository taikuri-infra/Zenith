...
┌─────────┐    ┌─────────────┐    ┌────────────────┐    ┌──────────────────┐
│         │    │             │    │                │    │                  │
│  Web UI │───▶│  Backend    │───▶│  Kubernetes    │───▶│  Zenith Operator │
...
Step 2: Backend (Go API)
  ...
  └── Backend creates CRD in Kubernetes:
      ...
      metadata:
        name: db-a1b2c3
        namespace: zenith-project-xyz
      spec:
        engine: postgresql
        version: "16"
        storage: 20Gi

Step 3: Zenith Operator (watching Database CRDs)
          ...
          storage:
            pvcTemplate:
              resources:
                requests:
                  storage: 20Gi
              # references the PVC Zenith created
...
| User Sees | Zenith CRD | Zenith Operator Creates | Service Operator | Hetzner Resource |
|-----------|-----------|------------------------|-----------------|-----------------|
...
| User Sees | Zenith CRD | Zenith Operator Creates | Hetzner Resource |
|-----------|-----------|------------------------|-----------------|
...
## Architectural Decisions (Frontend Development Phase)

The following decisions were made during frontend development and represent the finalized approach for key platform subsystems.

### API Gateway: Kong

Zenith uses **Kong with Kubernetes Operator (KongIngress CRDs)** as the primary API gateway.
...
**Key decisions:**
- Kong with KongIngress CRDs for declarative route management
- Integrated with Zenith Auth service for automatic JWT validation
- Route management per tenant: `/api/v1/*` routes to appropriate backend services
- Plugins enabled: `jwt-auth`, `rate-limiting`, `cors`, `request-transformer`, `ip-restriction`, `bot-detection`
- Each tenant's Gateway CRD translates to Kong configuration via the operator

### Auth Service: Built-in Keycloak-like (Zenith Auth)

Zenith Auth is a self-hosted, per-tenant authentication service (think Supabase Auth but self-hosted and multi-tenant).

**Key decisions:**
- Each tenant gets their own **realm** (similar to Keycloak realms)
- Supports **OpenID Connect + SAML** protocols
- Identity Providers: Google, GitHub, Azure AD, LDAP
- Pre-configured clients: `web-app`, `mobile-app`, `admin-panel`, `partner-api`
- JWT tokens validated automatically by Kong API Gateway
- User management features: MFA, roles, sessions per realm
- Simple UI exposed to users (no raw Keycloak admin console)

### IAM: Platform Identity & Access Management

IAM is **separate from Auth**. Auth handles tenant application users. IAM handles platform operator access.

**Key decisions:**
- API Keys with scopes: `read`, `write`, `deploy`, `registry:push`
- Team roles: `Owner`, `Admin`, `Developer`, `Viewer`
- SSO integration for platform operators (JumpCloud, Okta, Azure AD)
- IAM controls who can manage projects, deploy apps, and access the platform
- Auth controls who can log into tenant applications

### Monitoring Stack: Grafana + Loki + Prometheus

**Key decisions:**
- **Grafana** for dashboards with pre-built templates: Platform Overview, Service Health, Node Metrics
- **Prometheus** for metrics scraping - all services expose `/metrics` endpoints
- **Loki** for log aggregation - color-coded, filterable by service/level
- Pre-configured alerting rules for common scenarios (app down, high CPU, disk full, cert expiring)
- Each project gets auto-generated Grafana dashboards

### Container Registry: Built-in (like ECR)

**Key decisions:**
- Private registry per project: `registry.zenith.cloud/{project}/{image}`
- Image vulnerability scanning on push
- Lifecycle policies: keep last N tags, auto-delete untagged images
- Integrated with CI/CD pipeline (Kaniko pushes directly)
- Integrated with Kong for authenticated image pulls

### Cross-Tenant Communication

**Key decisions:**
- **Service Links:** Explicit opt-in connections between tenants
- **Shared Gateway:** Kong routes between tenants with separate consumer credentials
- **Event Bus:** NATS/Kafka for async communication between tenants
- **mTLS** for all cross-tenant traffic (Cilium/Linkerd)
- Cross-tenant access requires explicit authorization from both sides
...
│  │                                                                 │    │
│  │  ┌──────────────────────────────────────────────────────┐      │    │
│  │  │  Zenith API Server (Go)                               │      │    │
│  │  │                                                       │      │    │
│  │  │  - REST API for Web UI / CLI                          │      │    │
│  │  │  - Authentication (JWT + OAuth)                       │      │    │
│  │  │  - Authorization (RBAC)                               │      │    │
│  │  │  - Validates requests                                 │      │    │
│  │  │  - Creates/reads Zenith CRDs in K8s                   │      │    │
...
│  │                                                                 │    │
│  │  ┌─────────────────────────────────────────────────────────┐   │    │
│  │  │  Zenith Operator (Go, controller-runtime)                │   │    │
│  │  │                                                          │   │    │
│  │  │  Controllers:                                            │   │    │
...
## Zenith Operator Internal Design

The Zenith Operator is ONE binary with multiple controllers. Each controller watches one CRD type.
...
**CDKTF is used ONLY for initial cluster bootstrap** (`zenithctl install`). After that, the operator handles everything via hcloud-go.

---

## Installation Flow (zenithctl install)

```
$ zenithctl install --provider hetzner --token hc_xxxxx --region fsn1
...
[4/8] Creating control plane nodes (3x CX33)...                  ✓
      ├── zenith-cp-1 (10.0.1.1) - master
      ├── zenith-cp-2 (10.0.1.2) - master
      └── zenith-cp-3 (10.0.1.3) - master
...
[6/8] Installing infrastructure components...                     ✓
      ...
      ├── cert-manager
      └── Hetzner DNS webhook
[7/8] Installing Zenith platform...                               ✓
      ├── Zenith CRDs (20 types)
      ├── Zenith Operator
      ├── Zenith API Server
      ├── Zenith Web UI
...
════════════════════════════════════════════════════════════
  ...
  Dashboard:  https://console.zenith.dev  (or your IP)
  Admin:      admin@zenith.local
  Password:   xxxxxxxxxxxxxxxx

  Kubeconfig saved to: ~/.zenith/kubeconfig

  Monthly infrastructure cost: ~€17/mo (3x CX33 nodes)
  Zenith software cost: €0 (open source, Apache 2.0)
════════════════════════════════════════════════════════════
```

What `zenithctl install` does internally:
...
5. helm install / kubectl apply:
   ...
   - zenith (the platform Helm chart)

6. Zenith Helm chart installs:
   - Zenith CRDs
   - Zenith Operator deployment
   - Zenith API deployment
   - Zenith Web deployment
   - PostgreSQL (for Zenith's own DB - via CNPG)
...
zenith/
├── cmd/
│   ├── zenith-operator/          # Main operator binary
│   │   └── main.go               # controller-runtime manager
│   │
│   ├── zenith-api/               # API server binary
│   │   └── main.go               # Gin/Echo HTTP server
│   │
│   └── zenithctl/                # CLI binary
│       └── main.go               # cobra CLI
│
...
│   │
│   ├── installer/                # zenithctl install logic
...
Zenith/Zenith       Yes          Yes         YES         Yes       YES
────────────────────────────────────────────────────────────────────────
```

Zenith is the only project that is:
...
Every Hetzner resource has a Zenith CRD:

| Hetzner Resource | Hetzner API | Zenith CRD | User Sees |
|-----------------|-------------|-----------|-----------|
